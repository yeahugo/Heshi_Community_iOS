...
- (void)initTableView
{
    ...
    self.emojiButton.frame = CGRectMake(self.commentInputView.frame.size.width-40, 7.5, 25, 25);
    [self.emojiButton setBackgroundImage:[UIImage imageNamed:@"umeng_comm_emoji_bt"] forState:UIControlStateNormal];
    [self.commentInputView addSubview:self.emojiButton];
    [self.emojiButton addTarget:self action:@selector(showEmojis) forControlEvents:UIControlEventTouchUpInside];
    self.feedStyleDictionary = [NSMutableDictionary dictionaryWithCapacity:1];
...
- (void)showEmojis
{
    if (!self.emojiCollectionView) {
        ...
        self.emojiCollectionView = [[UMComCollectionView alloc]initWithFrame:CGRectMake(0, self.commentInputView.frame.origin.y+self.commentInputView.frame.size.height,self.window.frame.size.width,self.window.bounds.size.height-self.commentInputView.frame.origin.y-self.commentInputView.frame.size.height) collectionViewLayout:layout];
        __weak UMComFeedsTableView *weakSelft = self;
        self.emojiCollectionView.didSelectedAtImage = ^(NSString *imageName){
            NSMutableString *temString = [[NSMutableString alloc]initWithString:weakSelft.commentTextField.text];
            [temString insertString:imageName atIndex:weakSelft.commentTextField.text.length];
            weakSelft.commentTextField.text = temString;
            
       
            
            weakSelft.commentInputView.center = CGPointMake(weakSelft.emojiCollectionView.frame.size.width/2, weakSelft.emojiCollectionView.frame.origin.y-weakSelft.commentInputView.frame.size.height/2);
            weakSelft.commentTextField.center = CGPointMake(weakSelft.commentInputView.center.x-20, weakSelft.commentInputView.center.y);
       
        };
        [[UIApplication sharedApplication].keyWindow addSubview:self.emojiCollectionView];
    }
...
-(void)refreshFeedsData
{
    ...
    if ([self.feedTableViewController isKindOfClass:[UMComFeedTableViewController class]]) {
...
-(void)presentEditView:(NSString *)feedId
{
    ...
    self.commentTextField.frame = CGRectMake(0, self.commentTextField.frame.origin.y, self.commentTextField.frame.size.width-40, self.commentTextField.frame.size.height);
    [self.window addSubview:self.commentInputView];
    [self.window addSubview:self.commentTextField];
    self.commentFeedId = feedId;
...
- (void)keyboardWillShow:(NSNotification*)notification {
    ...
    float endheight = [[[notification userInfo] objectForKey:UIKeyboardFrameEndUserInfoKey] CGRectValue].size.height;
    self.commentInputView.frame = CGRectMake(0, self.commentInputView.frame.origin.y, self.frame.size.width, self.commentInputView.frame.size.height);
    self.commentTextField.frame = CGRectMake(10, self.commentTextField.frame.origin.y, self.commentInputView.frame.size.width-60, self.commentTextField.frame.size.height);
    
    self.commentInputView.center = CGPointMake(self.frame.size.width/2, self.window.frame.size.height - endheight - self.commentInputView.frame.size.height/2);
    self.emojiButton.frame = CGRectMake(self.commentInputView.frame.size.width-40, self.commentInputView.frame.size.height/2-self.commentTextField.frame.size.height/2, self.commentTextField.frame.size.height, self.commentTextField.frame.size.height);
    self.commentTextField.center = CGPointMake(self.commentInputView.center.x-20, self.commentInputView.center.y);
    
    self.emojiCollectionView.frame = CGRectMake(0, self.window.frame.size.height - endheight,self.window.frame.size.width,endheight);
